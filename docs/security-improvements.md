...
**Rule:** *auth-sessions.mdc* — "Access tokens: Prefer memory or short-lived HttpOnly cookies. Avoid localStorage when possible (XSS can steal)."
...
**Rule:** *csp-headers.mdc* — "Prefer a strict CSP; avoid `unsafe-inline` and `unsafe-eval`."
...
**Rule:** *csp-headers.mdc* — "Set X-Frame-Options: DENY or use CSP frame-ancestors 'none'."
...
**Rule:** *sensitive-data.mdc* — "Do not log sensitive data — No console.log, error reporting, or analytics with tokens, passwords, or PII."
*general-security.mdc* — "Do not expose stack traces or internal error details to the client in production."
...
**Rule:** *xss.mdc* — "Validate and sanitize URLs before `<a href>`. Allowlist `https://`; reject `javascript:`."
*react.mdc* — "Avoid `href={userUrl}` without validation; allowlist `https://` and reject `javascript:`."
...
**Rule:** *general-security.mdc* — "HTTPS only."
*api-network.mdc* — "Call APIs over HTTPS only."
...
**Rule:** *dependencies.mdc* — "Run npm audit (or equivalent) in CI; fix high/critical or document accepted risk."
...
**Rule:** *general-security.mdc* — "Do not expose stack traces or internal error details to the client in production."
...
**Rule:** *sensitive-data.mdc* — "No secrets in front-end."
